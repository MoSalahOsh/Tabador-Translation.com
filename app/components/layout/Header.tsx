...
export function Header({ lang, dict, site }: Props) {
  ...
  // For Arabic (RTL): the visual order of nav tabs from RIGHT-to-LEFT is:
  // Home → Services → Pricing → Urgent → FAQ → About → Contact
  // The browser flips this naturally for dir="rtl", so the array order is unchanged.
  // Home stays first in the array → renders rightmost in Arabic.
  const nav = [
    { key: 'home', href: `/${lang}` },
  ...
  return (
    ...
    >
      <div className="container mx-auto flex h-24 items-center justify-between px-4 md:px-6 gap-4 relative">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Link
            href={`/${lang}/contact`}
            className={cn(
              'hidden md:inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold transition-colors shadow-sm',
              transparent
      ...
      {open && (
        <div className="md:hidden border-t border-border bg-background/95 backdrop-blur-sm shadow-lg">
